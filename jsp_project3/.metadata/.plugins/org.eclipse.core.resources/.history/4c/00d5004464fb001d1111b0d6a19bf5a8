...
<body>
	...
	<br>
	<div align="center">
		<a href="/"><button align="center">메인화면으로</button></a> <br>
		<%-- 페이지네이션 위치 --%>
		...
		<c:if test="${pgh.prev}">
			<a
				href="/brd/page?pageNum=${pgh.startPage-1}&qty=${pgh.pgvo.qty}&type=${pgh.pgvo.type}&keyword=${pgh.pgvo.keyword}"><button>◀</button></a>
		</c:if>
		...
		<c:if test="${pgh.next}">
			<a
				href="/brd/page?pageNum=${pgh.endPage+1}&qty=${pgh.pgvo.qty}&type=${pgh.pgvo.type}&keyword=${pgh.pgvo.keyword}"><button>▶</button></a>
		</c:if>
	</div>
